...
void set_defaults(struct workerParams& Params)
{
    ...
    Params.myName = "default.db-cluster.tld";
    ...
    Params.log_failed_queries = true;
    Params.log_query_statistics = true;
...
void read_section_settings(struct workerParams& wParams, std::string secName, std::string confFile)
{
    set_defaults(wParams);
    INIReader reader(confFile);
    wParams.myName = secName;
    wParams.address = reader.Get(secName, "address", " ");
    wParams.username = reader.Get(secName, "user", " ");
    wParams.password = reader.Get(secName, "password", " ");
    wParams.socket = reader.Get(secName, "socket", "/var/lib/mysql/mysql.sock");
    wParams.database = reader.Get(secName, "database", "test");

    wParams.port = reader.GetInteger(secName, "port", 3310);
    wParams.threads = reader.GetInteger(secName, "threads", 1);
...
#endif
    wParams.verbose = reader.GetBoolean(secName, "verbose", true);
    ...
    wParams.log_failed_queries = reader.GetBoolean(secName, "log-failed-queries", true);
    wParams.log_query_statistics = reader.GetBoolean(secName, "log-query-statistics", true);
